function varargout = spider_plot_mod(P, varargin)
% Changes code in line 1168
...
if strcmp(axes_angular, 'on')
    ...
    for ii = 2:length(rho)
        ...
        h = plot(x_axes, y_axes,...
            'Color', axes_color);
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        % 04242020 YF: add to highlight the middel interval
        if ii==ceil((length(rho)+2)/2)
            h = plot(x_axes, y_axes,'--',...
...
%         if ii==2
%             h = plot(x_axes, y_axes,'-',...
%                 'Color', [192 0 0]/255,'linewidth',2);
%             h.Annotation.LegendInformation.IconDisplayStyle = 'off';
%         end   
%         if ii==length(rho)
%             h = plot(x_axes, y_axes,'-',...
%                 'Color', [46 117 182]/255,'linewidth',2);
%             h.Annotation.LegendInformation.IconDisplayStyle = 'off';
%         end   
        % Turn off legend annotation
    end
...
if ~strcmp(axes_labels, 'none')
    ...
    for ii = 1:length(axes_labels)
        ...
        % 05312022 YF: bold key element (hard coded)
        if ii==5
        text(x_pos, y_pos, axes_labels{ii},...
            'Units', 'Data',...
            'HorizontalAlignment', horz_align,...
